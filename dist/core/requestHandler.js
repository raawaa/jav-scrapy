...
class RequestHandler {
    ...
    async getPage(url, options = {}) {
        ...
        while (attempts < maxAttempts) {
            try {
                logger_1.default.debug(`开始请求页面: ${url} (尝试 ${attempts + 1}/${maxAttempts})`);
                ...
                if (statusCode === 302) {
                    ...
                    if (redirectUrl && redirectUrl.includes('driver-verify')) {
                        logger_1.default.error(`请求被重定向到年龄验证页面。这可能是由于请求头不完整被识别为机器人。`);
                        logger_1.default.error(`建议: 检查代理是否有效，或尝试在浏览器中访问 ${url} 确认是否可正常打开`);
                        return null;
                    }
                }
                // 403 友好提示
                if (statusCode === 403) {
                    logger_1.default.error(`请求被拒绝 (403): ${url}`);
                    logger_1.default.error(`这可能是由于以下原因:`);
                    logger_1.default.error(`  1. 代理 IP 被 Cloudflare 封禁 — 尝试更换代理或使用住宅代理`);
                    logger_1.default.error(`  2. 请求头不完整 — 更新 User-Agent 或使用 --cloudflare 模式`);
                    logger_1.default.error(`  3. 请求过于频繁 — 尝试增加延迟 (--delay) 或降低并发 (--parallel)`);
                    if (err.response?.data) {
                        const responseData = err.response.data;
                        if (typeof responseData === 'string' && responseData.length < 1000) {
                            logger_1.default.error(`响应内容: ${responseData.substring(0, 500)}`);
                        }
                    }
                }
                else {
                    ...
                    if (statusCode) {
                        logger_1.default.error(`响应状态码: ${statusCode}`);
                    }
                }
                // 判断是否可以重试
                ...
                if (isRetryable && attempts < maxAttempts - 1) {
                    ...
                    logger_1.default.warn(`将在 ${Math.round(delay / 1000)} 秒后重试 (${attempts}/${maxAttempts - 1})`);
                    await new Promise(resolve => setTimeout(resolve, delay));
                }
                else {
                    logger_1.default.error(`请求失败，已耗尽所有重试次数: ${url}`);
                    return null;
                }
            }
        }
