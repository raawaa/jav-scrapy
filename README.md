...
# 指定并发数和超时时间
$ jav pull -p 5 -t 60000 -o ~/downloads/magnets

# 搜索特定影片（使用 search 子命令）
$ jav search "影片关键词"
...
| 选项                 | 适用命令   | 说明                                     |
|----------------------|------------|------------------------------------------|
| -p, --parallel <num> | pull       | 设置并发连接数（默认：2）               |
| -t, --timeout <num>  | pull       | 设置连接超时时间（毫秒，默认：30000）   |
| -l, --limit <num>    | pull       | 设置抓取影片数量上限（默认：0不限）      |
| -o, --output <path>  | pull       | 设置结果保存路径（默认：当前工作目录）   |
| -b, --base <url>     | pull       | 自定义起始页URL                         |
| -x, --proxy <url>    | pull       | 使用代理服务器                          |
| -a, --allmag         | pull       | 抓取所有磁链（默认只抓取最大体积磁链）   |
| -N, --nopic          | pull       | 不抓取图片                              |
