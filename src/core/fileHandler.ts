...
class FileHandler {
  ...
  public async writeFilmDataToFile(data: FilmData): Promise<void> {
    ...
    try {
      ...
      if (duplicateIndex === -1) {
        const extractFilmId = (title: string): string | null => {
      ...
      if (duplicateIndex === -1) {
        ...
        if (normalizedNewTitle.length > 10) { // 只对较长的标题进行模糊匹配
          duplicateIndex = existingData.findIndex(item => {
            ...
            if (normalizedExistingTitle.length < 10) return false;
      ...
      logger.debug(`FileHandler: 检查重复数据，是否重复: ${isDuplicate}${isDuplicate ? ` (${duplicateReason})` : ''}`);
