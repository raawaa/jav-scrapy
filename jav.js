...
//var archiver = require('archiver');//下版会加入压缩截图文件夹为ZIP
...
function getSnapshots(link, snapshots) {
    ...
    let itemOutput = output + "/" + fanhao;//存储位置仍在名为番号的子文件夹，方便独立打包压缩
 	fs.access(itemOutput + ".rar", fs.F_OK, function (err) {//有rar时跳过，rar可自己打包，可用hamana浏览rar图包
		if (err) {
